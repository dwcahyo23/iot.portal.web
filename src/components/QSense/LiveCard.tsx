...
import { IconCheck, IconDeviceAnalytics, IconX } from "@tabler/icons-react";
import dayjs from "dayjs";
import { GaugeComponent } from "react-gauge-component";

const LiveCard = ({ comId, mcCd }: LiveCardProps) => {
  // Setup MQTT
  const topicKey = `qSense/Pub/${comId}/${mcCd}`;
  const { messages } = useMqtt([topicKey]);
  const topicData = messages[topicKey];
  const parsedMessage = topicData ? safeParseJSON(topicData.payload) : null;
